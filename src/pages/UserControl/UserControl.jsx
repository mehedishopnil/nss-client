                ...
                {filteredUsers?.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                                            ...
                                            <div className="flex items-center space-x-2 sm:space-x-3 py-2 sm:py-4">
                                                <div className="avatar placeholder">
                                                    <div className="bg-neutral text-neutral-content rounded-full w-8 h-8 sm:w-10 sm:h-10">
                                                        <span className="text-xs sm:text-sm">{u.name?.charAt(0) || 'U'}</span>
                                                    </div>
                                                </div>
                                                <div>
                                                    <div className="font-medium text-sm sm:text-base">{u.name || 'No name'}</div>
                                                    <div className="text-xs text-gray-500">ID: {u._id?.slice(-6)}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <div className="text-gray-700 text-sm sm:text-base">{u.email}</div>
                                        </td>
                                        ...
                                        <td>
                                            {editingUserId === u._id ? (
                                                <label className="cursor-pointer label p-0">
                                                    ...
                                                    />
                                                    <span className="ml-2 text-xs sm:text-sm">{newIsAdmin ? 'Admin' : 'User'}</span>
                                                </label>
                                            ) : (
                                                <span className={`badge badge-sm sm:badge-md ${u.isAdmin ? 'badge-primary' : 'badge-accent'}`}>
                        ...
                        <div className="sm:hidden space-y-3 p-2">
                            {filteredUsers?.map((u) => (
                                <div key={u._id} className="card bg-base-100 shadow-sm border border-gray-100">
                                    <div className="card-body p-3">
                                        <div className="flex justify-between items-start">
                                            <div className="flex items-center space-x-2">
                                                <div className="avatar placeholder">
                                                    <div className="bg-neutral text-neutral-content rounded-full w-8 h-8">
                                                        <span className="text-xs">{u.name?.charAt(0) || 'U'}</span>
                                                    </div>
                                                </div>
                                                <div>
                                                    <h3 className="font-medium text-sm">{u.name || 'No name'}</h3>
                                                    <p className="text-xs text-gray-500">{u.email}</p>
                                                </div>
                                            </div>
                                            <span className={`badge badge-sm ${u.isAdmin ? 'badge-primary' : 'badge-accent'}`}>
                                                {u.isAdmin ? 'Admin' : 'User'}
                                            </span>
                                        </div>
