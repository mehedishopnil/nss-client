...
import { FiShield, FiSearch, FiRefreshCw, FiUserX, FiUserCheck } from 'react-icons/fi';
import Loading from '../../components/Loading';
import { toast } from 'react-hot-toast';

const AdminControl = () => {
    const { user, allUsers, loading, updateUserRole } = useContext(AuthContext);
    ...
    const handleRemoveAdmin = async () => {
        ...
        try {
            ...
            await updateUserRole(userToRemove._id, { isAdmin: false });
    ...
    return (
        <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Admin Management</h1>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            {filteredAdmins.length > 0 ? (
                                filteredAdmins.map((admin) => (
                                    <tr key={admin._id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            {admin._id === user?._id ? (
                                                <span className="text-sm text-gray-500">Cannot remove yourself</span>
                                            ) : (
                                                <button
                                ...
                                <tr>
                                    <td colSpan="5" className="px-6 py-8 text-center text-gray-500">
                                        {searchTerm ? 'No matching admins found' : 'No administrators available'}
                                    </td>
                                </tr>
                            )}
                        </tbody>
                ...
                <div className="sm:hidden space-y-3 p-4">
                    {filteredAdmins.length > 0 ? (
                        filteredAdmins.map((admin) => (
                            <div key={admin._id} className="card bg-base-100 shadow-sm border border-gray-100">
            ...
            {userToRemove && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-xl p-6 max-w-md w-full">
                        <h3 className="text-lg font-semibold mb-4">Confirm Admin Removal</h3>
                        <p className="mb-4">
                            Are you sure you want to remove admin privileges from <span className="font-medium">{userToRemove.name}</span>?
