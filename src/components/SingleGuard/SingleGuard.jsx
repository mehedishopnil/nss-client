...
const SingleGuard = () => {
    ...
    useEffect(() => {
        if (allGuards && id) {
            ...
            if (foundGuard?.presence) {
                ...
                const todayRecord = foundGuard.presence.find(p => 
                    format(new Date(p.date), 'yyyy-MM-dd') === today
                );
    ...
    const totalSalary = guard.transactions
        ?.filter(t => t.type === 'salary')
        .reduce((sum, t) => sum + t.amount, 0) || 0;

    const totalAdvances = guard.transactions
        ?.filter(t => t.type === 'advance')
        .reduce((sum, t) => sum + t.amount, 0) || 0;

    const totalDeposits = guard.transactions
    ...
    const totalPresent = presenceRecords.filter(p => p.status === 'present').length;
    const totalAbsent = presenceRecords.filter(p => p.status === 'absent').length;
    const presencePercentage = presenceRecords.length > 0 
    ...
    const handleAddAdvance = async (e) => {
        ...
        try {
            // Here you would typically call an API to update the guard's transactions
    ...
    const handleMarkPresence = async (status) => {
        setIsSubmitting(true);
        try {
            // Here you would typically call an API to update the guard's presence
            // For now, we'll just simulate it
            ...
            const updatedPresence = (guard.presence || []).filter(p => 
                format(new Date(p.date), 'yyyy-MM-dd') !== format(today, 'yyyy-MM-dd')
            );
            
            updatedPresence.push(newPresence);
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-800 dark:text-white">{guard.name}</h1>
                    <p className="text-gray-600 dark:text-gray-400">ID: {guard._id}</p>
                </div>
                <div className="flex gap-3">
                    <button 
                        onClick={() => navigate(`/admin-panel/guard-transactions/${guard._id}`)}
                            ...
                            <div className="space-y-3">
                                ...
                                <div>
                                    ...
                                    <p className="font-medium">
                                        {guard.joinDate ? format(new Date(guard.joinDate), 'MMMM d, yyyy') : 'N/A'}
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="space-y-4">
                                <div className="stats shadow w-full">
                                    <div className="stat">
                                        ...
                                        <div className="stat-value text-primary">৳{totalSalary}</div>
                                    </div>
                                </div>
                                <div className="stats shadow w-full">
                                    <div className="stat">
                                        <div className="stat-title">Total Advances</div>
                                        <div className="stat-value text-secondary">৳{totalAdvances}</div>
                                    </div>
                                </div>
                                <div className="stats shadow w-full">
                                    <div className="stat">
                                        <div className="stat-title">Total Deposits</div>
                                        <div className="stat-value text-accent">৳{totalDeposits}</div>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-4">
                                <div className="stats shadow w-full">
                                    <div className="stat">
                                        ...
                                        <div className="stat-value text-success">{totalPresent}</div>
                                    </div>
                                </div>
                                <div className="stats shadow w-full">
                                    <div className="stat">
                                ...
                                {/* Today's Presence */}
                                <div className="mt-6">
                                    <h3 className="font-medium mb-3">Today's Status ({format(new Date(), 'MMMM d, yyyy')})</h3>
                                    <div className="flex gap-3">
                                        <button
                                            onClick={() => handleMarkPresence('present')}
                                            className={`btn flex-1 ${todayPresence === 'present' ? 'btn-success' : 'btn-outline'}`}
                                        ...
                                        <button
                                            ...
                                            className={`btn flex-1 ${todayPresence === 'absent' ? 'btn-error' : 'btn-outline'}`}
                            ...
                            <div className="overflow-x-auto">
                                <table className="table table-zebra">
                                    ...
                                    <tbody>
                                        ...
                                        {(!guard.transactions || guard.transactions.length === 0) && (
                                            <tr>
